...
// === ПРЕДЗАГРУЗКА GIF ДЛЯ МГНОВЕННОГО ПЕРЕКЛЮЧЕНИЯ ===
function preloadAbsGifs() {
    const gifs = ["abs_1.gif", "abs_2.gif", "abs_3.gif", "abs_4.gif", "abs_5.gif", "abs_6.gif", "abs_7.gif", "abs_8.gif", "abs_9.gif"];
    gifs.forEach(gifName => {
        const img = new Image();
        img.src = `gifs/${gifName}?t=${new Date().getTime()}`;
    });
}
...
function stopSpeaking() {
    if ('speechSynthesis' in window) { window.speechSynthesis.cancel(); window.speechSynthesis.pause(); window.speechSynthesis.cancel(); }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('input[name="complex-type"]').forEach(cb => { cb.addEventListener('change', updateComplexButton); });
});
...
function checkUserStatus() {
    ...
    if (profile) {
        ...
        if (data.level) showDashboard(data.name, data.level); else showWelcomeScreen(data.name);
...
function selectLevel(level) {
    ...
    profile.level = level; localStorage.setItem('fithero_profile', JSON.stringify(profile));
...
function startComplexWorkout() {
    ...
    if (selected.includes('abs')) { ex = ex.concat(BASE_WORKOUT_DATA.abs); preloadAbsGifs(); }
...
function renderExercise() {
    ...
    if (ex.isPlank) {
        ...
        setTimeout(() => startPlankTimer(targetValue), 1500);
    } else {
...
function startRestTimer() {
    ...
    document.getElementById('rest-timer-display').textContent = `${Math.floor(remaining / 60).toString().padStart(2, '0')}:${(remaining % 60).toString().padStart(2, '0')}`;
...
function startPlankTimer(seconds) {
    ...
    document.getElementById('rest-timer-display').textContent = `${Math.floor(seconds / 60).toString().padStart(2, '0')}:${(seconds % 60).toString().padStart(2, '0')}`;
...
function startRunningWorkout() { hideAllScreens(); document.getElementById('main-nav').classList.add('hidden'); document.getElementById('running-screen').classList.remove('hidden'); const profile = JSON.parse(localStorage.getItem('fithero_profile')); document.getElementById('running-distance-target').textContent = LEVEL_CONFIG[profile.level].runDistance; resetRunningUI(); speak(`Бег. Ваша цель на сегодня: ${LEVEL_CONFIG[profile.level].runDistance}. Нажмите старт.`); }
